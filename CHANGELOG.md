...
- **fix(b2b): tutti i fix Copilot review PR #186 (round 1 + 2)** — 16 issue risolte in 2 round di review: **Security**: open redirect bloccato per backslash/encoded variants in `auth.js`, WHERE guard `.eq('role','partner')` su handleRevoke, read-before-write su handleApprove per non demotare admin, rollback application a pending se profile update fallisce. **Performance**: `batchGetUserEmails` riscritto da O(totalUsers) listUsers a bounded parallel getUserById con concurrency=10 O(pageSize); denormalizzato `applicant_email` in `partner_applications` (migrazione 018). **Paginazione**: rimossa post-fetch filter che rompeva total/total_pages quando search attivo — DB-side `ilike` su display_name + totalCount DB per pagination. **Code quality**: tutti `var` → `const`/`let` in auth.js, supabase-config.js, business.js, admin.js (frontend + backend); CSS hex hardcoded → design tokens (--green, --red, --text-muted); checkAuth distingue 403 da errori server (retry button). **Falsy coercion**: `body.stake || null` → `body.stake ?? null` per preservare stake=0. **Test naming**: `match-insights.test.js` → `fixtures-h2h-form.test.js`, `user-bets.test.js` → `user-settings-bets.test.js`; error message assertion aggiornata per nuovi type h2h/form/odds-compare.
