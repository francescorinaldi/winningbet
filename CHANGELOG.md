...
- **fix(b2b): tutti i fix Copilot review PR #186 (round 1 + 2 + 3)** — 22 issue risolte in 3 round di review: **Security**: open redirect bloccato per backslash/encoded variants in `auth.js`, WHERE guard `.eq('role','partner')` su handleRevoke, read-before-write su handleApprove per non demotare admin, rollback application a pending se profile update fallisce; **RLS**: migrazione 019 — `partner_app_service_all` scoped a `service_role`, nuova policy `partner_app_insert_own` per authenticated con vincoli su status=pending e campi admin NULL. **Performance**: `batchGetUserEmails` riscritto da O(totalUsers) listUsers a bounded parallel getUserById con concurrency=10 O(pageSize); denormalizzato `applicant_email` in `partner_applications` (migrazione 018). **Paginazione**: rimossa post-fetch filter che rompeva total/total_pages quando search attivo — DB-side `ilike` su display_name + totalCount DB per pagination; `stats.total` ora usa query count globale dedicata (indipendente dal filtro search). **Resilienza**: `authFetch` in admin.js ora verifica Content-Type prima di parsare JSON, gestisce risposte non-JSON (es. Vercel 502/504 HTML) e errori di rete; `checkAuth` gestisce 401 con redirect a login. **Code quality**: tutti `var` → `const`/`let` in auth.js, supabase-config.js, business.js, admin.js (frontend + backend); CSS hex hardcoded → design tokens (--green, --red, --text-muted); `ADMIN_EMAILS` trim prima di filter per gestire spazi. **Admin safety**: guard self-demotion — admin non può rimuovere il proprio ruolo admin. **Falsy coercion**: `body.stake || null` → `body.stake ?? null` per preservare stake=0. **Test**: nuovo `admin.test.js` con 11 test (auth, resource, VAT validation, province, method); rename `match-insights.test.js` → `fixtures-h2h-form.test.js`, `user-bets.test.js` → `user-settings-bets.test.js`; error message assertion aggiornata per nuovi type h2h/form/odds-compare. **ESLint**: aggiunti globals `AbortSignal`, `alert`, `confirm`.
