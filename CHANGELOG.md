...
- **fix(b2b): tutti i fix Copilot review PR #186 (round 1–4)** — 28 issue risolte in 4 round di review: **Security (critical)**: migrazione 020 — `profiles_update_own` ora blocca modifica di `role` e `tier` via client (impedisce privilege escalation a admin); rimossa policy `partner_app_select_admin` (admin legge solo via service_role backend); open redirect bloccato per backslash/encoded variants in `auth.js`; WHERE guard `.eq('role','partner')` su handleRevoke; read-before-write su handleApprove; rollback application a pending se profile update fallisce. **RLS**: migrazione 019 — `partner_app_service_all` scoped a `service_role`, nuova policy `partner_app_insert_own` per authenticated con vincoli status=pending e campi admin NULL. **Data exposure**: rimosso join `tips(*)` da query `user_bets` (impedisce accesso a tip PRO/VIP via service key); GET `resource=apply` ora seleziona solo campi safe (esclusi notes, reviewed_by, reviewed_at). **Performance**: `batchGetUserEmails` riscritto da O(totalUsers) listUsers a bounded parallel getUserById con concurrency=10 O(pageSize); denormalizzato `applicant_email` (migrazione 018). **Paginazione**: rimossa post-fetch filter; `stats.total` usa query count globale dedicata. **Resilienza**: `authFetch` verifica Content-Type prima di JSON parse, gestisce non-JSON e errori di rete; `checkAuth` gestisce 401 con redirect a login. **Code quality**: tutti `var` → `const`/`let`; CSS hex → design tokens; `ADMIN_EMAILS` trim; admin self-demotion guard; `data-userId` su card utenti per feedback visivo. **Falsy coercion**: `body.stake || null` → `body.stake ?? null`. **Test**: `admin.test.js` con 11 test + save/restore `global.fetch`; rename test files per consistenza. **ESLint**: globals `AbortSignal`, `alert`, `confirm`.
