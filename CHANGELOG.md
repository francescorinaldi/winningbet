...
- **Swallowed errors in `script.js`** — Added `console.error` logging to four catch blocks that silently swallowed errors: `loadHomepageUserTier` (defaulted paying users to free tier without logging), `loadTrackRecord` (reset UI silently on API failure), and `loadTipsFromAPI` inner + outer catch blocks (completely swallowed fixtures fetch and API errors).
