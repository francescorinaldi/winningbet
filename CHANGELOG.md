...
- **feat(partner): landing page /partner — B2B Centro Scommesse** — `public/partner.html`: pagina statica completa con hero pitch, 3-layer model (licenza mensile €79–199 + revenue share 25% + esclusiva geografica 1,5 km), come funziona in-store (4 step), benefici, pricing cards con "Più Scelto" highlighted, simulazione guadagni (0/10/25 abbonati), form contatto mailto pre-compilato, conformità ADM, footer con disclaimer 18+. Cattura `?ref=CODE` nell'URL e salva in `localStorage('wb_ref_code')` per il tracking referral. Mobile responsive, usa il design system (`styles.css` + Space Grotesk/Inter).

- **feat(referral): tracking conversioni programma Centro Partner** — `supabase/migrations/017_add_referral_code.sql`: aggiunge `referral_code TEXT UNIQUE` (assegnato ai partner) e `referred_by TEXT` (partner che ha portato l'utente) alla tabella `profiles`, con indici parziali. `api/user-settings.js`: nuovo `resource=referral` (POST idempotente) — salva `referred_by` solo se non già impostato (un utente non può cambiare partner di provenienza). `public/auth.js`: cattura `?ref=CODE` dall'URL query string e salva in `localStorage` prima del redirect OAuth. `public/dashboard.js`: nuova funzione `saveReferralIfPresent()` chiamata dopo l'auth — invia il ref code all'API e rimuove da `localStorage`.

- **feat(telegram): card tip più visual e shareabile** — `api/_lib/telegram.js`: aggiunta funzione `formatMatchDate()` (data italiana compatta: "Sab 01 Mar · 20:45"). `formatTipBlock()` ora accetta `leagueFlag` e `leagueName` come parametri — aggiunge riga header con flag campionato + data/ora all'interno di ogni tip block (per shareability come screenshot standalone). Odds e edge in grassetto. `formatDigest()` passa flag e nome a ogni `formatTipBlock`.

- **feat(stats): track record breakdown per campionato e per fascia di quota** — `api/stats.js`: nuove funzioni `buildLeagueBreakdown()` (win rate + ROI per ogni lega) e `buildOddsRangeBreakdown()` (win rate + ROI per fasce 1.20–1.50, 1.50–2.00, 2.00–3.00, 3.00+). Aggiunti al response body come `by_league` e `by_odds_range`. Entrambi pubblici (nessun gate auth).

### Fixed

- **fix(stats): track record mostra won E lost nei risultati recenti** — `public/script.js`: rimosso il filtro `status === 'won'` sui risultati recenti nella sezione Track Record. La lista mostra ora tutte le tip chiuse (won + lost) nell'ordine dell'API. Trasparenza fondamentale per la credibilità — un track record cherry-picked con sole vittorie viene percepito come inaffidabile.
...
- **fix(dashboard): initialize particles canvas** — Added `initParticles({ maxParticles: 30, densityDivisor: 25, connections: false })` in `dashboard.js` so the existing `<canvas id="particles">` element is no longer invisible. Lightweight config for visual consistency with other pages.
...
- **feat(engine) #160 — Market Waterfall in prediction-engine.js** — Implementazione del waterfall nel motore API legacy (`api/_lib/prediction-engine.js`): (1) costante `MARKET_WATERFALL_ORDER` con i 14 mercati in ordine di priorità; (2) regola #13 nel system prompt che istruisce l'AI a esplorare tutti i mercati prima di skippare; (3) istruzione waterfall nel prompt utente che permette fino a 2 candidate per partita; (4) funzione `deduplicateByBestEV` che seleziona la prediction con EV massimo quando l'AI propone alternative per la stessa partita. Log migliorati: il mercato fallito è ora visibile nel warning. 10 nuovi test (33 totali).
